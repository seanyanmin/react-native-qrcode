var bgColorString = 'ffffff' 
var fgColorString = '000000'
...
var qrcode = function() {
  ...
  /**
   * qrcode
   * @param bgColor  //0 to 255
   * @param fgColor  //0 to 255
   */
  var qrcode = function(bgColor, fgColor) {

    var PAD0 = 0xEC;
    var PAD1 = 0x11;

    if(convertToFullColorCode(bgColor).length == 6){

      bgColorString = convertToFullColorCode(bgColor);
    }
    if(convertToFullColorCode(fgColor).length == 6){

      fgColorString = convertToFullColorCode(fgColor);
    }

    
    var _typeNumber = 4;
    var _errorCorrectionLevel = QRErrorCorrectionLevel['L' ];
    var _modules = null;
    var _moduleCount = 0;
    var _dataCache = null;
    var _dataList = [];
    ...
    _this.addQRCodeData = function(data, mode) {
    ...
    _this.buildDataWithURL = function(cellSize, margin) {
      ...
      return buildDataWithURL(size, size, function(x, y) {
    ...
    _this.createImgTag = function(cellSize, margin, alt) {
      ...
      img += '<img';
      img += '\u0020src="';
      img += _this.buildDataWithURL(cellSize, margin);
      img += '"';
      img += '\u0020width="';
      img += size;
      img += '"';
   ...
   * @param numChars
   */
  qrcode.changeStringToBytes = function(unicodeData, numChars) {
  ...
  var gifImage = function(width, height) {
    ...
    _this.write = function(out) {
      ...
      // black
       ...
       out.writeByte(parseInt(fgColorString.substring(0,2).slice(0),16));
       out.writeByte(parseInt(fgColorString.substring(2,4).slice(0),16));
       out.writeByte(parseInt(fgColorString.substring(4,6).slice(0),16));
      ...
      //  out.writeByte(0xff);
       ...
       out.writeByte(parseInt(bgColorString.substring(0,2).slice(0),16));
       out.writeByte(parseInt(bgColorString.substring(2,4).slice(0),16));
       out.writeByte(parseInt(bgColorString.substring(4,6).slice(0),16));
  ...
  var buildDataWithURL = function(width, height, getPixel) {
...
!function(qrcode) {
  ...
  qrcode.stringToBytesFuncs['SJIS'] = qrcode.changeStringToBytes(
